...
TRTModule::TRTModule(const std::string &onnx_file) {
    ...
    assert((context = engine->createExecutionContext()) != nullptr);
    assert((input_idx = engine->getBindingIndex("input")) == 0);
    assert((output_idx = engine->getBindingIndex("output-topk")) == 1);
    ...
    assert(cudaMalloc(&device_buffer[input_idx], input_sz * sizeof(float)) == 0);
    assert(cudaMalloc(&device_buffer[output_idx], output_sz * sizeof(float)) == 0);
    assert(cudaStreamCreate(&stream) == 0);
...
void TRTModule::build_engine_from_onnx(const std::string &onnx_file) {
    ...
    assert((engine = builder->buildEngineWithConfig(*network, *config)) != nullptr);
    config->destroy();
    parser->destroy();
    network->destroy();
    builder->destroy();
